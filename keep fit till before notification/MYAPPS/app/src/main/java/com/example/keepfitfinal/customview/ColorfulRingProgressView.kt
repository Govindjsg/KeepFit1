package com.example.keepfitfinal.customview
...
internal class ColorfulRingProgressView(private val mContext: Context, attrs: AttributeSet?) : View(mContext, attrs) {
    private var mPercent = 0f
    private var mStrokeWidth = 0f
    private var mBgBorderColor = -0x1e1e1f
    var mBgFillColor= Color.parseColor("#F5F5F5")
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        if(!isHidePercent) {
            ...
            canvas.drawText(percent.toInt().toString() + "%", width / 2.toFloat(), yPos.toFloat(), textPaint)
        }
    ...
    override fun onSizeChanged(w: Int, h: Int, oldw: Int, oldh: Int) {
        ...
        mShader = LinearGradient(mOval!!.left, mOval!!.top, mOval!!.left, mOval!!.bottom, mFgColorStart, mFgColorEnd, Shader.TileMode.MIRROR)
    }
    ...
    private fun updateOval() {
        ...
        mOval = RectF(paddingLeft + mStrokeWidth, paddingTop + mStrokeWidth, paddingLeft + (width - xp) - mStrokeWidth, paddingTop + (height - yp) - mStrokeWidth)
    }
    ...
    init {
        val a = mContext.theme.obtainStyledAttributes(attrs, R.styleable.ColorfulRingProgressView, 0, 0)
        try {
            mBgBorderColor = a.getColor(R.styleable.ColorfulRingProgressView_bgBorderColor, -0x1e1e1f)
            ...
            mBgFillColor =a.getColor(R.styleable.ColorfulRingProgressView_bgFillColor, mBgFillColor)
            isHidePercent = a.getBoolean(R.styleable.ColorfulRingProgressView_hideText,isHidePercent)
            ...
            mStrokeWidth = a.getDimensionPixelSize(R.styleable.ColorfulRingProgressView_strokeWidth, dp2px(6f)).toFloat()
            if(hideBorder){
